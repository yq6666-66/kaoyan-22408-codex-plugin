...
本政策适用于 `kaoyan-408` Skills 插件。插件由公开的静态文件组成，不提供发布者控制的 App、MCP、后台服务、账号系统、模型代理或云端数据存储。用户可以选择让具备本地文件权限的宿主连接其自行配置的 Obsidian Vault，或连接用户自己授权的 Notion 工作区、Udemy 课程、Sider Scholar 检索与 GoodNotes 笔记。
...
- 网页搜索结果、GitHub 真题来源、Notion 页面 ID、Obsidian 配置、Udemy 登录态、Sider Scholar 会话或 GoodNotes 导出数据。
