...
本政策适用于 `kaoyan-22408` Skills 插件。插件由公开的静态文件组成，不提供发布者控制的 App、MCP、后台服务、账号系统、模型代理或云端数据存储。用户可以选择让具备本地文件权限的宿主连接其自行配置的 Obsidian Vault，或连接用户自己授权的 Notion 工作区。
...
启用 Notion 后，当前宿主通过用户已授权的 Notion 工具搜索、读取和更新用户指定页面。插件发布者不接收 Notion Token、页面内容、会话记录或学习状态，也没有独立访问用户工作区的权限。默认只保存从对话中提炼的最小、可复用学习增量；用户可以使用“本次不记忆”“只读模式”或“不要同步 Notion”禁止本轮写入。
